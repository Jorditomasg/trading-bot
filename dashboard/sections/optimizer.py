...
def optimizer_section(db: Database) -> None:
    ...
    # ── Trail Stop Optimizer ──────────────────────────────────────────────────
    st.divider()
    st.markdown("## Trail Stop Optimizer")
    st.caption(
        f"Tests {len(ACTIVATION_GRID) * len(TRAIL_GRID)} combinations of trail activation × "
        "trail distance ATR multipliers. EMA TP/SL fixed at current approved values."
    )

    _trail_pending_banner(db)

    col_trail_form, col_trail_results = st.columns([1, 1], gap="large")

    with col_trail_form:
        with st.form("trail_optimizer_form"):
            cfg = db.get_runtime_config()

            c1, c2 = st.columns(2)
            with c1:
                sym_default = cfg.get("symbol", "BTCUSDT")
                trail_symbol = st.selectbox(
                    "Symbol", _SYMBOLS,
                    index=_SYMBOLS.index(sym_default) if sym_default in _SYMBOLS else 0,
                    key="trail_symbol",
                )
                tf_default = cfg.get("timeframe", "4h")
                trail_tf = st.selectbox(
                    "Timeframe", _TIMEFRAMES,
                    index=_TIMEFRAMES.index(tf_default) if tf_default in _TIMEFRAMES else 2,
                    key="trail_tf",
                )
            with c2:
                trail_period = st.selectbox("Lookback", _PERIODS, index=1,
                                            format_func=lambda d: f"{d} days",
                                            key="trail_period")
                trail_risk_pct = st.number_input(
                    "Risk / Trade (%)", min_value=0.1, max_value=5.0,
                    value=round(float(cfg.get("risk_per_trade", 0.01)) * 100, 1),
                    step=0.1, format="%.1f", key="trail_risk",
                )
                trail_cost_pct = st.number_input(
                    "Fee / side (%)", min_value=0.0, max_value=1.0,
                    value=0.07, step=0.01, format="%.2f", key="trail_cost",
                )

            trail_run = st.form_submit_button(
                f"▶  Run Trail Optimizer ({len(ACTIVATION_GRID) * len(TRAIL_GRID)} combos)",
                use_container_width=True, type="primary",
            )

    if trail_run:
        _run_trail_optimizer(
            db, trail_symbol, trail_tf, trail_period,
            trail_risk_pct / 100, trail_cost_pct / 100,
        )

    with col_trail_results:
        if "trail_results" in st.session_state:
            _display_trail_results(st.session_state["trail_results"])
        else:
            st.markdown("## Results")
            st.markdown(
                "<div style='color:#333;font-size:0.75rem;letter-spacing:0.1em;margin-top:2rem'>"
                "Run the trail optimizer to see the heatmap here.</div>",
                unsafe_allow_html=True,
            )
...
# ── Trail pending banner ──────────────────────────────────────────────────────

def _trail_pending_banner(db: Database) -> None:
    pending = db.get_best_pending_trail_run()
    if pending is None:
        return

    cfg       = db.get_runtime_config()
    cur_act   = float(cfg.get("trail_act_mult",  1.0))
    cur_trail = float(cfg.get("trail_atr_mult",  1.5))

    pf_color = GREEN if pending["profit_factor"] >= 1.2 else WHITE

    st.markdown(
        f"<div style='border:1px solid {GREEN};padding:12px 16px;margin-bottom:1rem'>"
        f"<span style='font-size:0.65rem;letter-spacing:0.18em;color:{GREEN}'>● TRAIL PENDING PROPOSAL</span><br>"
        f"<span style='font-size:0.75rem;color:#F5F5F5'>"
        f"Activation {pending['trail_activation_mult']:.2f}× ATR &nbsp;·&nbsp; "
        f"Trail dist {pending['trail_atr_mult']:.2f}× ATR &nbsp;·&nbsp; "
        ...
        f"</span><br>"
        f"<span style='font-size:0.6rem;color:#555'>"
        f"Current: act {cur_act:.2f} · trail {cur_trail:.2f} &nbsp;—&nbsp; "
        f"{pending['symbol']} {pending['timeframe']} · {pending['period_days']}d lookback"
        f"</span>"
        f"</div>",
        unsafe_allow_html=True,
    ...
    with col_a:
        if st.button("✓ Apply Trail Config", type="primary", use_container_width=True,
                     key="trail_approve"):
            db.set_runtime_config(
                trail_atr_mult=str(pending["trail_atr_mult"]),
                trail_act_mult=str(pending["trail_activation_mult"]),
            )
            db.set_trail_run_status(pending["id"], "approved")
            st.success(
                f"Applied: activation {pending['trail_activation_mult']:.2f}× "
                f"· trail {pending['trail_atr_mult']:.2f}×. "
                "Takes effect on next position_manager tick."
            )
            st.rerun()
    with col_r:
        if st.button("✕ Reject", use_container_width=True, key="trail_reject"):
            db.set_trail_run_status(pending["id"], "rejected")
            st.info("Trail proposal rejected — current config unchanged.")
            st.rerun()


# ── Trail history table ───────────────────────────────────────────────────────

def _trail_history_table(db: Database) -> None:
    runs = db.get_trail_runs(limit=20)
    if not runs:
        st.caption("no trail optimization runs yet")
        return
    ...
    for r in runs:
        ...
        rows.append({
            ...
            "SYMBOL":     r["symbol"],
            "TF":         r["timeframe"],
            "ACTIVATION": f"{r['trail_activation_mult']:.2f}×",
            "TRAIL DIST": f"{r['trail_atr_mult']:.2f}×",
            "PF":         f"{r['profit_factor']:.2f}",
            "WR":         f"{r['win_rate']:.1f}%",
            "DD":         f"{r['max_drawdown']:.1f}%",
            "TRADES":     str(r["total_trades"]),
...
def _run_trail_optimizer(db, symbol, timeframe, period_days, risk, cost):
    progress_placeholder = st.empty()
    completed = [0]
    viable    = [0]

    def on_progress(idx, total, activation, trail, summary):
        completed[0] = idx
        if summary is not None:
            viable[0] += 1
        pct = idx / total
        progress_placeholder.progress(
            pct,
            text=f"Testing act={activation:.2f} trail={trail:.2f} … {idx}/{total} · {viable[0]} viable",
        )

    with st.spinner("Fetching historical data…"):
        try:
            results = run_trail_grid_search(
                db=db,
                symbol=symbol,
                timeframe=timeframe,
                lookback_days=period_days,
            ...
            st.error(f"Trail optimizer error: {exc}")
            progress_placeholder.empty()
            return
...
def _display_trail_results(results: list[dict]) -> None:
    if not results:
        st.warning("No viable trail configurations found. Try a longer lookback.")
        return

    viable = [r for r in results if r["viable"]]
    best   = results[0]

    activations = sorted(set(r["activation_mult"] for r in results))
    trails      = sorted(set(r["trail_mult"]       for r in results))
    pf_map = {(r["activation_mult"], r["trail_mult"]): r["profit_factor"] for r in results}

    z = []
    for act in activations:
        row = []
        for tr in trails:
            pf = pf_map.get((act, tr), 0.0)
            row.append(pf if pf != float("inf") else 5.0)
        z.append(row)

    fig = go.Figure(go.Heatmap(
        z=z,
        x=[f"Trail {t}" for t in trails],
        y=[f"Act {a}"   for a in activations],
        colorscale=[[0, "#1A1A1A"], [0.4, "#FF0000"], [0.7, "#888"], [1.0, "#F5F5F5"]],
        zmin=0.8, zmax=2.0,
        text=[[f"{v:.2f}" for v in row] for row in z],
        texttemplate="%{text}",
    ...
    for r in top:
        pf = r["profit_factor"]
        rows.append({
            "ACTIVATION": f"{r['activation_mult']:.2f}×",
            "TRAIL":      f"{r['trail_mult']:.2f}×",
            "PF":         f"{pf:.2f}",
            "WR":         f"{r['win_rate']:.1f}%",
            "DD":         f"{r['max_drawdown']:.1f}%",
            "TRADES":     str(r["total_trades"]),
    ...
    if viable:
        st.success(
            f"Best: activation {best['activation_mult']:.2f}× · trail {best['trail_mult']:.2f}× "
            f"→ PF {best['profit_factor']:.2f}"
        )
